...
openssl req -new -keyout multi-topic-client.key -nodes -newkey rsa:2048 -subj "/CN=Apple Push Services: com.eatthepath.pushy/UID=com.eatthepath.pushy" | \
    openssl x509 -extfile ./apns-extensions.cnf -extensions apns_multi_topic_client_extensions -req -CAkey ca.key -CA ca.pem -days 36500 -set_serial 1 -sha512 -out multi-topic-client.pem
...
do
  ...
  keytool -genkeypair -keyalg EC -validity 36500 -storepass pushy-test -keypass pushy-test -dname "CN=com.eatthepath.pushy.{$i}" -keystore multiple-keys.jks -alias "pair${i}"
done
