...
openssl req -new -keyout not-apns.key -nodes -newkey rsa:2048 -subj "/CN=Not APNs/UID=com.example.not-apns" | \
    openssl x509 -req -CAkey ca.key -CA ca.pem -days 36500 -set_serial $RANDOM -sha512 -out not-apns.pem
...
openssl pkcs12 -export -in not-apns.pem -inkey not-apns.key -out not-apns.p12 -password pass:pushy-test
...
do
  ...
  keytool -genkeypair -storepass pushy-test -keypass pushy-test -dname "CN=Apple Push Services: com.relayrides.pushy.{$i}" -keystore multiple-keys.jks -alias "pair${i}"
done
...
rm ca.key multi-topic-client.key single-topic-client.key not-apns.key multiple-keys.jks
